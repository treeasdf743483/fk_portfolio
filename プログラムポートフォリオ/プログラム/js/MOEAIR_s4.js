...
function motion() {
	...
	if(b==0){
		...
		console.log(cnt);
			for(var i = 0;i <= 3;i++){
				...
				f3();
				k1[i] = delt*f[i]/6;
				x_arr[i] = x_arr[i]+ k1[i]*0.5;
				f3();
				k2[i] = delt*f[i]/3;
				//console.log(k2[i]+"  kkk")
				x_arr[i] = x_arr[i]+ k2[i]*0.5;
				f3();
				k3[i] = delt*f[i]/3;
				x_arr[i] = x_arr[i]+ k3[i];
				f3();
				k4[i] = delt*f[i]/6;
				x_arr[i]=x_arr[i]+k4[i];
		...
		//ここまで
		console.log("vx55555555555555"+x_arr[0]);
		console.log("x5555555555555"+x_arr[2]);
		//console.log("t"+t);
	...
	y_md = height - x_arr[1];
	cnt++;
	draw(x_later,y_later,x_arr[0],y_md);
	x_later = x_arr[0];
	y_later = y_md;
	outWrite(x_arr[0], x_arr[1], x_arr[2], x_arr[3], t, cnt);
...
//console.log("f3"+vm);
	...
	f[1]=x_arr[1];
		if (x_arr[0] >= 0) {
			f[2] = -b * Math.abs(x_arr[0] * x_arr[0]);
		} else {
			f[2] = b * Math.abs(x_arr[0] * x_arr[0]);
		}
		if (x_arr[1] >= Math.sqrt(Math.abs(m*gm/b))) {
			f[3] = -ms * gm - b *Math.abs(x_arr[1] * x_arr[1]);
		} else {
			f[3] = -ms * gm + b * Math.abs(x_arr[1] * x_arr[1]);
		}
	}
