...
//m入力 左下にふっきれる 原因　初期値　エラー204行目 b空条件ひっかかる outWrite成功　最初血がy v**2爆発　y爆発　java参照 数値エラー
var x_init = 100;
var y_init = 300;
var width = 500;
var height = 800;
...
function motion() {
	...
	if(b==0){
		...
		return k1[i]+k2[i]+k3[i]+k4[i];
			...
			y=x_arr[1];
			//ここまで
			console.log("vx"+v_arr[0]);
			console.log("x"+x_arr[0]);
			//console.log("t"+t);
		}
	...
	draw(x_later,y_later,x_arr[0],y_md);
			x_later = x_arr[0];
			y_later = y_md;
			outWrite(v_arr[0], v_arr[1], x_arr[0], x_arr[1], t, cnt);
